...
).query('domain == "EUR-12"').query('experiment.str.contains("eval")')
...
for study in sorted(list(studies)):
  dreq_study = dreq.query('priority.str.contains(@study)').copy()
  ...
  print(f'Total {study:14} study estimated data request size is: {np.nansum(size_TB.values):8.3f} TB')
